export const createImportStatements = filenames => {
  return filenames
    .map((file, i) => `import * as importFns${i + 1} from '${file}';`)
    .join(``);
};
  ...
  return importFiles
    .map((importFile, i) => {
      ...
      const ret = `${defaultStatement || ''}${
        namedImports.length ? `${namedStatement}` : ''
      }`;
      return ret;
    })
    .join(``);
...
export const createNamedDescribes = fns => {
  return `${Object.keys(fns.namedFns).map(namedFn => {
    return `describe('${namedFn}', () => {${createIt(
      namedFn,
      fns.namedFns[namedFn]
    )}});`;
  }).join(`
`)}`;
};
...
export const createIt = (name, fns) => {
  return `it('returns true when ${fns.join(', ')} is true', () => {${
    fns.length ? fns.map(fn => `mockFunction(${fn}, true);`).join(``) : ''
