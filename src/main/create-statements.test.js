...
describe('createAssignMockStatements', () => {
  it('returns correct string', () => {
    expect(subjectUnderTest.createAssignMockStatements(importFiles, fns))
      .toEqual(`const isNumberSub = importFns1.default;
const { isStringSub } = importFns1;`);
  });
});

describe('createDescribes', () => {
  it('returns correct string', () => {
    expect(subjectUnderTest.createDescribes(fns)).toEqual(
...
describe('createIt', () => {
  it('returns correct string', () => {
    expect(subjectUnderTest.createIt('jam', ['sub1', 'sub2']))
      .toEqual(`it('returns true when sub1, sub2 is true', () => {
    mockFunction(sub1, true);
    mockFunction(sub2, true);

    expect(subjectUnderTest.jam()).toEqual(true);
  });`);
  });
});
